...
def score_memory(memory: Any, query: str, entity_names: set[str] | None = None, query_embedding: list[float] | None = None) -> float:
    content = str(_field(memory, "content", ""))
    importance = float(_field(memory, "importance", 0.4))
    confidence = float(_field(memory, "confidence", 0.8))
    ...
    query_terms = {term.casefold() for term in TOKEN.findall(query) if len(term) > 2}
    ...
    entity_match = 1.0 if names & entities else 0.0
    ...
    semantic = cosine_similarity(query_embedding, _field(memory, "embedding"))
    return 0.46 * lexical + 0.27 * max(0.0, semantic) + 0.12 * entity_match + 0.1 * importance + 0.05 * recency * confidence


def rank_memories(memories: list[Any], query: str, entity_names: set[str] | None = None, limit: int = 8, query_embedding: list[float] | None = None) -> list[Any]:
