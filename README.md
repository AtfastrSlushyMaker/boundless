...
### Experimental fully local split model setup

On an Apple Silicon Mac, the existing Qwen3.5 9B Abliterated MLX model can remain the **narrator** while a separate Ollama model handles **State tracking**. In **Model settings → Advanced · AI roles**, select [`richardyoung/qwen2.5-3b-instruct-abliterated:latest`](https://ollama.com/richardyoung/qwen2.5-3b-instruct-abliterated) to experiment with this split. **This specific 3B model is not recommended for persistent campaigns yet:** it passed only 4/12 Tristan state checks versus 11/12 for the local 9B on the same 20 recorded turns, and a smaller prompt did worse. In a live test it saved a newly copied spell as an item; its summary attempt also required deterministic fallback. [See the measured comparison](docs/local-state-model-benchmark-2026-09-25.md). Keep state and summary on the 9B, or use another model that passes your own regression scenario. Other providers and one-model configurations remain available.

```bash
ollama pull richardyoung/qwen2.5-3b-instruct-abliterated:latest
```

Use `http://127.0.0.1:11434` for native development or `http://host.docker.internal:11434` from the Docker API. On the Mac Docker stack, Boundless also maps a saved Ollama loopback URL to the host gateway. Set the state role to a low temperature (0–0.1) and an 8192-token context to start. Summary can inherit from state; canon repair can inherit from narrator; hosted state fallback remains off unless enabled explicitly. The selected characters, aliases, relationships, items, abilities, objectives, and relevant lore are sent to the state model with the action and final narration. The model proposes JSON changes; validation, entity resolution, reconciliation, and CanonGuard retain authority over the saved world. The summary job falls back to a deterministic factual summary after repeated failures. Check `/api/health` for separate narrator, state, and summary availability. With local embeddings (the default) and your own ComfyUI endpoint, ordinary gameplay does not need a hosted model.
